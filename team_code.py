...
def train_challenge_model(data_folder, model_folder, verbose):
    ...
    features_meta = features.iloc[:,:8]
    features_eeg = features.iloc[:,8:]
    scaler = StandardScaler()
    features_eeg = scaler.fit_transform(features_eeg)
    features_eeg = pd.DataFrame(features_eeg)
    features = pd.concat([features_meta,features_eeg],axis=1)   
    features.columns = np.arange(features.shape[1])
    outcome_model , cpc_model = train_model(features,outcomes,cpcs)
    # Save the models.
    save_challenge_model(model_folder, scaler, imputer, outcome_model, cpc_model)
...
def run_challenge_models(models, data_folder, patient_id, verbose):
    ...
    features_meta = features.iloc[:,:8]
    features_eeg = features.iloc[:,8:]
    features_eeg = scaler.transform(features_eeg)
    features_eeg = pd.DataFrame(features_eeg)
    features = pd.concat([features_meta,features_eeg],axis=1)
    features.columns = np.arange(features.shape[1])
...
def save_challenge_model(model_folder, scaler, imputer, outcome_model, cpc_model):
    d = {"scaler":scaler, 'imputer': imputer, 'outcome_model': outcome_model, 'cpc_model': cpc_model}
    filename = os.path.join(model_folder, 'models.sav')
...
def get_features(data_folder, patient_id):
    ...
    features1 = get_eeg_features(eeg_data)
    # Extract features.
    return np.hstack((patient_features,features1.flatten() ))
...
def get_eeg_features(eeg_data):
    if eeg_data is None:
        return float("nan")*np.ones(235)
    num_channels, num_samples = eeg_data.shape
    num_trials = 1 # Number of trials
   
    # Create a 3D array to store the EEG data for each trial
    eegData = np.zeros((num_channels, num_samples, num_trials))
   
    # Fill the 3D array with the same data for each trial
    for trial in range(num_trials):
        eegData[:, :, trial] = eeg_data
    fs = 100
    #Shannon Entropy
    ShannonRes = shannonEntropy(eegData, bin_min=-200, bin_max=200, binWidth=2)
    # Subband Information Quantity
    # delta (0.5–4 Hz)
    eegData_delta = filt_data(eegData, 0.5, 4, fs)
    ShannonRes_delta = shannonEntropy(eegData_delta, bin_min=-200, bin_max=200, binWidth=1)
    # theta (4–8 Hz)
    eegData_theta = filt_data(eegData, 4, 8, fs)
    ShannonRes_theta = shannonEntropy(eegData_theta, bin_min=-200, bin_max=200, binWidth=1)
    # alpha (8–12 Hz)
    eegData_alpha = filt_data(eegData, 8, 12, fs)
    ShannonRes_alpha = shannonEntropy(eegData_alpha, bin_min=-200, bin_max=200, binWidth=1)
    # beta (12–30 Hz)
    eegData_beta = filt_data(eegData, 12, 30, fs)
    ShannonRes_beta = shannonEntropy(eegData_beta, bin_min=-200, bin_max=200, binWidth=1)
    # gamma (30–100 Hz)
    eegData_gamma = filt_data(eegData, 30, 40, fs)
    ShannonRes_gamma = shannonEntropy(eegData_gamma, bin_min=-200, bin_max=200, binWidth=2)
   
   
    # δ band Power
    bandPwr_delta = bandPower(eegData, 0.5, 4, fs)
    # θ band Power
    bandPwr_theta = bandPower(eegData, 4, 8, fs)
    # α band Power
    bandPwr_alpha = bandPower(eegData, 8, 12, fs)
    # β band Power
    bandPwr_beta = bandPower(eegData, 12, 30, fs)
    # γ band Power
    bandPwr_gamma = bandPower(eegData, 30, 40, fs)
    std_res = eegStd(eegData)
    # α/δ Ratio
    ratio_res = eegRatio(eegData,fs)
    # Regularity (burst-suppression)
    regularity_res = eegRegularity(eegData,fs)
    ...
    feature_arr  = np.vstack(feature_list)
    fg = feature_arr.reshape(-1,)
    return fg
